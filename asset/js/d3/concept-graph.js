var CollapsibleTree = function(elt) {
    ...
    var itemHeight = 55;
    var svgWidth = 460;
    var svgHeight = itemHeight * 4.286;
    var gap = itemHeight / svgHeight;

    var m = [10, 10, 10, 10],
        w = svgWidth - m[1] - m[3],
        h = svgHeight * 4 - m[0] - m[2],
        i = 0,
        root,
        root2;
    ...
    var vis = d3.select(elt).append("svg:svg")
        ...
        .attr("style", "border: 1px #ccc solid;height: " + svgHeight + "px;overflow: hidden;")
        .append("svg:g")
        //.attr("transform", "translate(" + m[3] + "," + m[0] + ")")//; // left-right
        //.attr("transform", "translate(" + m[0] + "," + m[3] + ")")//; // top-bottom
        .attr("transform", "translate(0," + h / 2 + ")")
        .attr("transform", "rotate(90) translate(6,-" + svgWidth / 2 + ")")
    ...
    var that = {
        ...
        updateBoth: function(source) {
            ...
            var addText = function(nodeEnter, that, root, textMore) {
                ...
                .attr("x", function(d) {
                    ...
                    if (that.isParent(d)) {
                        ...
                        return d.children || d._children ? -n : n;
                    }
                })
                    .attr("dy", (textMore != undefined) ? "1.55em" : ".35em")
                ...
                .attr("text-anchor", function(d) {
                    ...
                    .attr("transform", function(d) {
                        if (d != root) {
                            if (that.isParent(d)) {
                                return "rotate(-90) translate(100,0)";
                            } else {
                                return "rotate(-90) translate(-100,0)";
                            }
                        } else {
                            return "rotate(-90) translate(45,-30)";
                        }
                    })
                    .text(function(d, i) {
                        //console.log('d',i,nodeEnter[0][i]);
                        //var current = nodeEnter[0][i];
                        currentText = d.name;
                        return (textMore != undefined) ? textMore : d.name;
                    })
                    .style("fill-opacity", 1);
                return currentText;
            };
            addText(nodeEnter, that, root);
            //addText(nodeEnter, that, root, "ok");
            /*nodeEnter.append("svg:text")
              // .attr("x", function(d) { return d.children || d._children ? -10 : 10; })
              .attr("x", function(d) {
                  var n = 2;
              ...
              .attr("transform", function(d) {
                  if (d != root) {
                      ...
                      return "rotate(-90) translate(45,-20)";
                  }
              })
              .text(function(d) {
                  return d.name;
